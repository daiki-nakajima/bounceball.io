...
class Screen {
    ...
    render(iTimeCurrent) {
        ...
        if(null !== this.aTank) {
            ...
            const iIndexFrame = parseInt(fTimeCurrentSec / 0.2) % 6;  // 画像のフレーム番号
    ...
    renderTank(tank, iIndexFrame) {
        ...
        this.context.drawImage(this.assets.imageBalls,
            this.assets.rectBallsImage[iIndexFrame].sx, this.assets.rectBallsImage[iIndexFrame].sy,	// 描画元画像の右上座標
            this.assets.rectBallsImage[iIndexFrame].sw, this.assets.rectBallsImage[iIndexFrame].sh,	// 描画元画像の大きさ
            -SharedSettings.TANK_WIDTH * 0.5,	// 画像先領域の右上座標（領域中心が、原点になるように指定する）
            -SharedSettings.TANK_HEIGHT * 0.5,	// 画像先領域の右上座標（領域中心が、原点になるように指定する）
            SharedSettings.TANK_WIDTH,	        // 描画先領域の大きさ
            SharedSettings.TANK_HEIGHT);	    // 描画先領域の大きさ
